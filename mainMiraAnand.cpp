...
const int INITIAL_LINE_MAX = 3; // represents the maximum # of cars that can join the initial line, will be used to generate a random # of cars between 1-3 in main()
...
const int REAR_CAR_SWITCHES = 15; // represents the probability that a rear car will switch lanes
const int CAR_JOINS_EMPTY_LANE = 50; // represents the probability that a new car will join a lane if it is completely empty and there is still more time in the simulation
const int TIME_PERIODS = 20; // represents the # of time periods the simulation will run for

int main()
{
    srand(time(0)); // needed as the first line in main() for randomization purposes
    ...
    for (int i = 0; i < TOLL_BOOTH_LANE_NUM; i++) // for each one of the lanes
    {
        ...
        // use a range-based for loop and the "auto" keyword to output the Car objects in each one of the 4 lanes
        // traverse through the array of type deque<Car>
    ...
    for (int timePeriod = 1; timePeriod <= TIME_PERIODS; timePeriod++) // simulation will run for 20 time periods & then terminate
    {
        ...
        for (int i = 0; i < TOLL_BOOTH_LANE_NUM; i++) // creation of another inner for loop to handle each one of the lanes
        {
            // creating 3 if statements so that probability events can be handled independently from one another
            // this means that during a time period, there is a possibility that all events can occur - a head car leaving, a new car joining, and a rear car switching lanes, which is an accurate situation for a toll booth
            // if a new car joins the end of one line and that same car decides to switch lanes in the same time period, this can mimic a situation where the car realizes that it may be better off in a different line that is being processed faster
            // - or the line they have switched into is not operational at the moment and they need to immediately switch to a different one
            // there is also a possibility that no events occur during a time period - no head car leaves, no new car joins, and no rear car switches
            // - this mimics a situation where a head car may not leave because there is a delay when processing payment at the toll booth and a new car simply may not join because there are no incoming cars. Also, no rear car may want/need to switch
            
            // handle the case where the car at the head of the line pays & leaves
            // use .empty() to ensure that the lane is not empty before proceeding with head car removal
            if (!tollBoothLanes[i].empty() && probability <= HEAD_CAR_PAYS_LEAVES)
            {
                ...
                // this is valid because the if statement performs a check to ensure that we will not remove from an empty deque
                cout << "Lane: " << i + 1 << " Paid: ";
            ...
            // will also handle the case in which the line is empty, but there is still more time in the simulation - there will be a 50/50 probability of a new car joining the line if the line is completely empty
            ...
            // handle the case where a rear car will shift lanes to a random lane that is not their own
            probability = rand() % 100 + 1; // generate probability for this case
            // use .empty() to ensure that the lane is not empty before proceeding with switching of a rear car
            if (!tollBoothLanes[i].empty() && probability <= REAR_CAR_SWITCHES)
            {
                ...
                // create a do-while loop so that a random lane # will be generated until one is generated that does not match the one the car is currently in
                do
                {
        ...
        for (int i = 0; i < TOLL_BOOTH_LANE_NUM; i++) // for each one of the lanes
        {
            cout << "Lane " << i + 1 << " Queue:" << endl; // output the lane #
            
            if (tollBoothLanes[i].empty()) // use .empty() to check if the lane has any more cars in line
            {
                // if there are no cars in the lane's line, the queue is empty
                cout << "    Empty" << endl;
            }
            else // display the cars in the lane's line
            {
                for (auto car : tollBoothLanes[i]) // use a range-based for loop and the "auto" keyword to output the Car objects in each one of the 4 lanes
                {
                    cout << "    ";
                    car.print(); // using the .print() nethod in the Car class to print each Car object stored in the deque
                }
